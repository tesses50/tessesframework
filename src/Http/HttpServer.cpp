...
void ServerSentEvents::SendCustomEvent(const std::string &etype,
...
class WSServer {
  public:
    ...
    void Start() {
        this->closed = false;
        std::string key;
        if (ctx->requestHeaders.TryGetFirst("Sec-WebSocket-Key", key) &&
            !key.empty()) {
            key.append("258EAFA5-E914-47DA-95CA-C5AB0DC85B11");
            auto res = Crypto::Sha1::ComputeHash((const uint8_t *)key.c_str(),
                                                 key.size());
...
std::string ServerContext::GetOriginalPathWithQuery() {
    ...
    return this->originalPath + "?" +
           HttpUtils::QueryParamsEncode(this->queryParams);
}
...
void ServerContext::SendServerSentEvents(
    std::shared_ptr<ServerSentEvents> sse) {
    ...
    if (strm == nullptr || this->method == "HEAD")
        return;
    sse->mtx.Lock();
...
parseSection(ServerContext *ctx, std::string boundary,
    ...
    if (!req.TryGetFirst("Content-Type", ct))
        ct = "application/octet-stream";
    if (req.TryGetFirst("Content-Disposition", cd0) &&
        ContentDisposition::TryParse(cd0, cd1)) {
        if (cd1.filename.empty()) {
            std::shared_ptr<MemoryStream> ms =
                std::make_shared<MemoryStream>(true);
            ...
            bool retVal =
                parseUntillBoundaryEnd(ctx->GetStream(), strm, boundary);
...
HttpServer::HttpServer(
    std::shared_ptr<Tesses::Framework::Streams::TcpServer> tcpServer,
    std::shared_ptr<IHttpServer> http, bool showIPs, bool debug) {
...
HttpServer::HttpServer(uint16_t port, std::shared_ptr<IHttpServer> http,
                       bool showIPs, bool debug)
    : HttpServer(std::make_shared<TcpServer>(port, 10), http, showIPs, debug) {}

HttpServer::HttpServer(std::string unixPath, std::shared_ptr<IHttpServer> http,
                       bool debug)
    : HttpServer(std::make_shared<TcpServer>(unixPath, 10), http, false,
                 debug) {
    this->showARTL = true;
}
...
std::shared_ptr<Stream> ServerContext::OpenResponseStream() {
    ...
    this->WriteHeaders();
    auto strm = std::make_shared<HttpStream>(this->strm, length, false,
                                             version == "HTTP/1.1");
    if (method == "HEAD") {
        strm->Close();
    }
...
std::shared_ptr<Stream> ServerContext::OpenRequestStream() {
    ...
    if (!this->requestHeaders.TryGetFirstInt("Content-Length", length))
        length = -1;
    return std::make_shared<HttpStream>(this->strm, length, true,
                                        version == "HTTP/1.1");
}
void HttpServer::StartAccepting() {
    fflush(stdout);
    if (http == nullptr || server == nullptr)
        return;
    ...
    thrd = new Threading::Thread([svr, http, serverPort, debug]() -> void {
        while (TF_IsRunning()) {
            ...
            Threading::Thread thrd2(
                [sock, http, ip, port, serverPort, debug]() -> void {
    ...
    if (this->showIPs) {
        ...
        for (auto _ip : NetworkStream::GetIPs()) {
            StdOut() << "\x1B[32m" << _ip.first << ": "
                     << "\x1B[35mhttp://" << _ip.second << ":"
                     << (uint64_t)this->GetPort() << "/" << NewLine();
        }
    }
    if (this->showARTL) {
        if (!svr->IsValid())
...
void ServerContext::SendErrorPage(bool showPath) {
    ...
    std::string errorHtml =
        showPath ? ("<html><head><title>File " +
                    HttpUtils::HtmlEncode(this->originalPath) + " " +
                    HttpUtils::StatusCodeString(this->statusCode) +
                    "</title><meta name=\"color-scheme\" content=\"dark "
                    "light\"></head><body><h1>" +
                    std::to_string((int)this->statusCode) + " " +
                    HttpUtils::StatusCodeString(this->statusCode) +
                    "</h1><h4>" + HttpUtils::HtmlEncode(this->originalPath) +
                    "</h4></body></html>")
                 : ("<html><head><title>" + std::to_string(this->statusCode) +
                    " " + HttpUtils::StatusCodeString(this->statusCode) +
                    "</title><meta name=\"color-scheme\" content=\"dark "
                    "light\"></head><body><h1>" +
                    std::to_string((int)this->statusCode) + " " +
...
void HttpServer::Process(std::shared_ptr<Stream> strm,
                         ...
                         bool debug) {
    TF_LOG("In process");
    std::shared_ptr<BufferedStream> bStrm =
        std::make_shared<BufferedStream>(strm);
...
void ServerContext::StartWebSocketSession(
    ...
    std::shared_ptr<CallbackWebSocketConnection> wsc =
        std::make_shared<CallbackWebSocketConnection>(onOpen, onReceive,
                                                      onClose);

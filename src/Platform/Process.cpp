...
static void escape_windows_args(std::string &str,
...
// TODO: Implement for WIN32
#if defined(_WIN32)
        if (this->strm == NULL || this->eos && writing)
            return 0;
    ...
    !defined(TESSESFRAMEWORK_ENABLE_PROCESS)
        ...
        if (this->strm < 0 || this->eos && writing)
            return 0;
...
Process::Process(std::string name, std::vector<std::string> args,
                 bool includeThisEnv)
    : Process(name, args, std::vector<std::pair<std::string, std::string>>(),
              includeThisEnv) {}
Process::Process(std::string name, std::vector<std::string> args,
...
#else
    int r;
    if (waitpid(this->hidden.GetField<ProcessData *>()->pid, &r, WNOHANG) !=
        -1) {
        this->exited = true;
        this->exitCode = r;
        return r;
...
Process::Process(std::string name, std::vector<std::string> args,
                 std::vector<std::string> env, bool includeThisEnv)
    : Process(name, args, std::vector<std::pair<std::string, std::string>>(),
              includeThisEnv) {
...
bool Process::Start() {
    ...
    if (this->redirectStdIn) {
        if (!CreatePipe(&p->si.hStdInput, &p->stdin_strm, &attr, 0))
            return false;
    ...
    if (!CreateProcessW(
            (LPCWSTR)u16_name.c_str(), (LPWSTR)u16_args.data(), NULL, NULL,
            (this->redirectStdIn || this->redirectStdOut ||
             this->redirectStdErr),
            CREATE_UNICODE_ENVIRONMENT, (LPVOID)env.c_str(),
            workDir.empty() ? (LPCWSTR)NULL : (LPCWSTR)workDir.c_str(),
            &(p->si), &(p->pi))) {
        if (this->redirectStdIn) {
            CloseHandle(p->stdin_strm);
            CloseHandle(p->si.hStdInput);
...
*/
    ...
    if (pid == 0) {
        ...
        if (!this->workingDirectory.empty())
            Tesses::Framework::Filesystem::VFSPath::SetAbsoluteCurrentDirectory(
...
void Process::Kill(int signal) {
    ...
    if (signal != SIGKILL && signal != SIGTERM)
        std::cout << "WARN: We terminated the process" << std::endl;
    ...
    if (signal == SIGTERM) {
        ...
        TerminateProcess(this->hidden.GetField<ProcessData *>()->pi.hProcess,
                         -1);
...
std::shared_ptr<Tesses::Framework::Streams::Stream> Process::GetStdinStream() {
    ...
    return nullptr;
#else
    return std::make_shared<ProcessStream>(
        this->hidden.GetField<ProcessData *>()->stdin_strm, true);
#endif
}
...
#else
    return std::make_shared<ProcessStream>(
        this->hidden.GetField<ProcessData *>()->stdout_strm, false);
#endif
}
...
#else
    return std::make_shared<ProcessStream>(
        this->hidden.GetField<ProcessData *>()->stderr_strm, false);
#endif
}
